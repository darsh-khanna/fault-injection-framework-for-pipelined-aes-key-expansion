...
// =============================================================================
// AES Key Expansion - Non-Pipelined (Sequential FSM)
// Supports AES-128 (key_size=2'b00), AES-192 (key_size=2'b01), AES-256 (2'b10)
//
// Key input packing convention (UNIFIED with pipelined module):
...
//
// Output bus packing:
//   round_keys_out[1919:0] holds 15 round keys (enough for AES-256 RK00..RK14).
//   Slot i (0-based) occupies bits [i*128+127 : i*128].
//   AES-128: slots 0..10 valid  (11 keys: RK00..RK10)
//   AES-192: slots 0..12 valid  (13 keys: RK00..RK12)
//   AES-256: slots 0..14 valid  (15 keys: RK00..RK14)
//
// This module passed all NIST KAT baseline checks (NP_errs=0) in prior
// simulation; no functional changes from v3.
// =============================================================================

module aes_key_expansion_nonpipelined (
    input  logic          clk,
    input  logic          rst_n,
    input  logic [255:0]  key_in,
    input  logic [1:0]    key_size,
    input  logic          key_valid_in,
    output logic [1919:0] round_keys_out,
    output logic          keys_valid_out,
    output logic [3:0]    rounds_out
);

    // =========================================================================
    // Parameters
    // =========================================================================
    wire [5:0] total_words = (key_size == 2'b00) ? 6'd44 :
                             (key_size == 2'b01) ? 6'd52 : 6'd60;
    wire [5:0] Nk          = (key_size == 2'b00) ? 6'd4  :
                             (key_size == 2'b01) ? 6'd6  : 6'd8;
    wire [3:0] Nr          = (key_size == 2'b00) ? 4'd10 :
                             (key_size == 2'b01) ? 4'd12 : 4'd14;

    // =========================================================================
    // Word storage - 60 words for AES-256
    // =========================================================================
    logic [31:0] words [0:59];
    logic [5:0]  word_count;
    logic        busy;
    logic [3:0]  Nr_latched;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            word_count     <= 6'd0;
            Nr_latched     <= 4'd0;
            for (int k = 0; k < 60; k++) words[k] <= 32'h0;

        end else if (key_valid_in && !busy) begin
            case (key_size)
                2'b00: begin
                    words[0] <= key_in[127:96];  words[1] <= key_in[95:64];
                    words[2] <= key_in[63:32];   words[3] <= key_in[31:0];
                end
                2'b01: begin
                    words[0] <= key_in[191:160]; words[1] <= key_in[159:128];
                    words[2] <= key_in[127:96];  words[3] <= key_in[95:64];
                    words[4] <= key_in[63:32];   words[5] <= key_in[31:0];
                end
                default: begin
                    words[0] <= key_in[255:224]; words[1] <= key_in[223:192];
                    words[2] <= key_in[191:160]; words[3] <= key_in[159:128];
                    words[4] <= key_in[127:96];  words[5] <= key_in[95:64];
                    words[6] <= key_in[63:32];   words[7] <= key_in[31:0];
                end
            endcase
            word_count     <= Nk;
            busy           <= 1'b1;
            ...
            if (word_count < total_words) begin
                ...
                automatic logic [5:0]  wmod = word_count % Nk;
                automatic logic [5:0]  wdiv = word_count / Nk;

                if (wmod == 6'd0)
                    temp = subword(rotword(words[word_count - 1])) ^ rcon(wdiv[3:0]);
                else if ((Nk == 6'd8) && (wmod == 6'd4))
                    temp = subword(words[word_count - 1]);
                else
                    temp = words[word_count - 1];

                words[word_count] <= temp ^ words[word_count - Nk];
                word_count        <= word_count + 6'd1;
            end else begin
                keys_valid_out <= 1'b1;
                rounds_out     <= Nr_latched;
                busy           <= 1'b0;
            end
    ...
    // =========================================================================
    // OUTPUT PACKING
    // Slot i = round_keys_out[(i*128)+127 -: 128]:
    //   words[i*4]   → bits [i*128+127 -: 32]
    //   words[i*4+1] → bits [i*128+95  -: 32]
    //   words[i*4+2] → bits [i*128+63  -: 32]
    //   words[i*4+3] → bits [i*128+31  -: 32]
    // =========================================================================
    always_comb begin
        round_keys_out = '0;
        for (int rk = 0; rk <= 14; rk++) begin
            automatic int base_word = rk * 4;
            if (base_word + 3 < 60) begin
                round_keys_out[(rk*128 + 127) -: 32] = words[base_word];
                round_keys_out[(rk*128 +  95) -: 32] = words[base_word + 1];
                round_keys_out[(rk*128 +  63) -: 32] = words[base_word + 2];
                round_keys_out[(rk*128 +  31) -: 32] = words[base_word + 3];
            end
        end
    end
